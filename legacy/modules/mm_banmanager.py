...
class BanManager(object):
	...
	def __init__(self, modManager):
		...
		self.__epocRe = re.compile(r'^:\d+(\.\d)?$')
		self.__banPeriodRe = re.compile(r'^:?\d+(\.\d)?$')
	...
	def validateBanAddress(self, address):
		...
		# N.B.  We could do some extra parsing here
		# WARNING: If we do add extra checks here ensure they are compat with unban
	...
	def validateBanPeriod(self, banPeriod=None):
		...
		if self.__epocRe.search(banPeriod) is not None:
			return banPeriod
	...
	def __addBan(self, method, nick, period, address, cdkeyhash, profileid, by, reason, datetime, skipSave=False):
		...
		# Note: we may loose presision here with the casts to int for bf2.Timer but
		# not much we can do
		if 'Round' == ban['period']:
			# Round ban
			...
			expires_from_now = float(ban["period"].split(":")[1]) - now			
	...
	def banPlayer(self, player, banReason=None, banPeriod=None, banType=None, banMethod=None, bannedBy=None, banDelay=None):
		...
		# N.B.  We store a copy of key player info in case they leave before the ban
		# activates
		try:
	...
	def banPlayerNow(self, player=None, banReason=None, banPeriod=None, banType=None, banMethod=None, bannedBy=None):
		...
		try:
			...
			ban = self.__addBan(player.mmBanDetails['method'],
	...
	def onGameStatusChanged(self, status):
		...
		try:
			...
			# BF2 reloads banlist.con from disk on change so correct what its got by
			# clearing
	...
	def onPlayerDisconnect(self, player):
		...
		# Note: we dont remove bans scheduled for this player as they will be
		# successful
